...
| 网页语音条播放器（接管 Mastodon 原生播放器） | 线上仍是 v14（三项故障：手机波形退化为等高占位条、PC 完全没接管、手机播不出声）。**v15 已实现但未部署**：observer 回调改为只藏不建、波形采样改为可重试（A 的根因）、藏法由 `display:none` 改为裁剪使元素留在渲染树（C 最像的机制）、`play()` 的 rejection 不再静默。复现环境已补真实音频与外观断言，静置实测 `natives 0 / gap 0 / 7 个播放器全部有真实波形 / 89 根柱子 / listeners 1`，桌面 Chromium 点播放出声。**B（PC 不接管）仍未定位**，新增 `window.__piVoiceDebug()` 供真机一次性定位断点。iOS 真机、真实 MP3 解码耗时均未验证。详见 [`docs/clip-brain/VOICE_PLAYER_HANDOFF.md`](docs/clip-brain/VOICE_PLAYER_HANDOFF.md)（临时交接单，收口后并回本文件并删除）。录音 → 上传 → 转写 → 回填这条链不受影响 |
| 独立 CMX 前端 | 计划中 |
| 公共联邦 | 永不实施 |
