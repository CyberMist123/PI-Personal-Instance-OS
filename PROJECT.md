...
| R18 真实样本声音形状修正 | 2026-08-20 用 Owner 明确授权的 `呻吟+喘息.mp4` 跑生产全链路：Qwen text=`嗯嗯嗯嗯。`；Gemini 检出 3 段 moan 后转 2 段 sigh，后两段音高偏低。旧 renderer 把重复段压成裸 `[moan]`/`[sigh]` 且走向退化为「说话 → 有声呼气」，已确认不符合 #39。修正为逐段保留质感、标明续接/转类，并输出段数、间隔再现、音高移动与类别变化组成的「声音形状」。 |
| R18 Qwen Omni 快速主路（本地待部署） | 2026-08-20 本地接入 `qwen3.5-omni-plus` 优先分析，严格白名单清洗后复用确定性 renderer；无效或空事件回退 Gemini。独立提示词为 `mcp/prompts/qwen_r18_nvv.md`。真实样本实测约 6–17 秒；嘈杂样本空结果会触发 fallback。新增声音形状组合规则，可固定输出「笑声 → 强而短促的咳嗽 → 突然吸气 → 低位拖长发声 → 持续快速运动喘息」。相关回归 41 passed；尚未部署生产。 |
| 公共联邦 | 永不实施 |
