...
- 网页悬浮录音键 v3「秒发语音 + 后台补文字」（`cmx_mcp.voice_widget` + `GET /files/voice.js` + `POST /files/transcribe` + `PUT /api/v1/statuses/<id>` 编辑 + Nginx `sub_filter` 注入）已通过云端 Linux 自动测试（119 passed），未在目标 Windows/真机实测：**需真机验证 iOS Safari 的 `audio/mp4` 录制**（MediaRecorder 支持面与实际可播放性）以及 **Nginx reload**（`sub_filter` 生效、`proxy_set_header Accept-Encoding ""` 后上游未压缩、Cloudflare 侧仍正常压缩）；另需实测：**Mastodon 4.6.3 的编辑 API 是否接受 `media_attributes` 里的 `description`**（这是补 alt 的唯一路径）、真实录音的转写耗时是否在 90 秒超时内、`cmx-mcp-http` **在设置 `CMX_WHISPER_MODEL_DIR` 后重启**才能看到该变量（否则永远补不上文字）、编辑后网页与原生 App 的显示与「已编辑」标记、页面提前关闭时帮工回帖兜底的链路，以及注入脚本不与 Mastodon 前端样式/Service Worker 冲突。原生 App 客户端不会加载该脚本。
