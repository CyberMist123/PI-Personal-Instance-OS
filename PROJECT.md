...
- 帮工 v1 与中文语音转写（SQLite v5 `worker_done`、`cmx-worker` 入口、`worker-*.ps1`）当前代码已通过全套 合并后全套测试，未在目标 Windows 实测：部署需在 `mcp\` 重装 `pip install -e .[workers]`（新增 OpenCC、要求 faster-whisper >=1.1），准备含非空 `model.bin` 的 CTranslate2 模型目录并设置 `CMX_WHISPER_MODEL_DIR`，然后同时重启 worker 与 `cmx-mcp-http`。仍需用 Owner 真实普通话录音比较字错率、专有词命中率、首次/后续转写耗时与内存；`direct`/`self` 对帮工账号不可见的边界不变。
- 网页悬浮录音键 v17（`cmx_mcp.voice_widget` + `cmx-voice-outbox`）当前代码已通过全套 合并后全套测试 与 Node 语法检查，未部署到目标 Windows/真机。v5 曾在目标 Windows 完成 Nginx reload、HTTP 重启和 loopback 检查，但不能作为 v17 验收。部署后需确认公网 `GET /files/voice.js` ETag 为 `voice-17`，再分别在 iOS Safari 与 Windows 浏览器验证：第二次点麦克风即上传、离线/HTTP 失败后重开页面续传、幂等重传不重复发帖、IndexedDB 成功后清理、`audio/mp4`/WebM、Mastodon 编辑正文与 alt、真实中文转写耗时，以及清除站点数据会删除未发送 outbox 的预期行为。
...
| 独立 CMX 前端 | 计划中 |
| 网页录音 / 本机中文转写 v6 | 已实现/未实测：本地自动测试 120 passed；目标 Windows、iOS/Windows 浏览器与真实普通话字错率待验收 |
| 公共联邦 | 永不实施 |
