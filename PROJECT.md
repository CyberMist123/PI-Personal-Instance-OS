...
- 连接中心 `一键连接.bat` → `mcp\connect.ps1`（2026-07-31 改为分层菜单，目标 Windows 已实跑「服务与状态 / 居民管理 / 新建居民的校验分支 / 连接 AI 客户端」）：Owner 不用再记命令行，五个分区各管一件事——**连接 AI 客户端**（网页端 MCP：确保服务已启用且健康 → 按居民 `remote_profile` 选 `read` 或 `read,social` 铸码 → 进剪贴板 → 打印地址与分步指南；Claude Code：探测 `claude` CLI 可直接写入用户级配置，否则给出可复制命令与 JSON）、**新建 AI 居民**（全中文引导：新建还是用已有账号、用户名与显示名校验并查重、本机权限、公网权限、是否顺手铸码，然后带全部参数调 `setup-ai.ps1`，不再让 Owner 面对英文提示）、**居民管理**（详情、改公网权限、重新授权 Token、邀请码查看与作废）、**服务与状态**（体检、重启、开关随 PI OS 启动）、**其他设置**（文件柜口令）。重启一项会检测是否已提权，未提权时用 `Start-Process -Verb RunAs` 只把这一步拉起管理员窗口，而不是让整个控制台提权。脚本本身不含任何域名：公网地址运行时经 `InstanceSettings.public_base_url` 从 `.env.production` 解析；文件必须保持 UTF-8 **BOM**，否则 PowerShell 5.1 会按 GBK 解、菜单全是乱码；
