...
- 连接中心 `一键连接.bat` → `mcp\connect.ps1`（2026-07-30，目标 Windows 已实跑菜单 1/2/5：邀请码铸造、Claude Code 命令生成、状态体检均正常；菜单 3/4/6 未实跑）：Owner 不用再记命令行。菜单包含「网页端 MCP 授权」（自动确保远程服务已启用且健康 → 按居民 `remote_profile` 选 `read` 或 `read,social` 铸码 → 邀请码进剪贴板 → 打印服务器地址与分步指南）、「Claude Code 本机接入」（探测 `claude` CLI，可直接写入用户级配置，失败则给出可复制的 `claude mcp add` 命令与 JSON）、「Owner 文件柜口令」、「重新授权居民 Mastodon Token」（从数据库回填现有 profile/audience/capability，避免重新授权时被重置）、「状态体检」与「重启远程 MCP」。脚本本身不含任何域名：公网地址运行时经 `InstanceSettings.public_base_url` 从 `.env.production` 解析；文件为 UTF-8 BOM，以便 PowerShell 5.1 正确读中文；
